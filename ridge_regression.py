...
for i in range(len(alpha_ridge)):
    ...
    for train_index, test_index in kcv.split(x):
        ...
        R2.append(r2_score(y_test, np.round(RR.predict(x_test), 2)))
        RMSE.append(sqrt(mean_squared_error(y_test, np.round(RR.predict(x_test), 2))))
    print(f'For alpha = {alpha_ridge[i]}:')
    print(f'Ridge Regression R-squared: {np.mean(R2):.5f}')
    ...
    print(f'Predicted Y (From Ridge Regression) R-squared: {pred_R2:.5f}')
    print(f'Predicted Y (From Ridge Regression) RMSE: {pred_RMSE:.5f}')
    print('')
